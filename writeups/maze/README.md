# Maze — Reverse Engineering Writeup

> **Challenge — "Please try to understand this binary and solve it!"**
>
> *"Welcome to the maze! Please type your input:"*
>
...
| **SHA-256** | *(download from source — not bundled in this repo)* |
| **MD5** | *(download from source — not bundled in this repo)* |
...
5. [Entry Point — The Only Call That Matters](#3-entry-point--the-only-call-that-matters)
6. [Dissecting the Maze Node](#4-dissecting-the-maze-node)
7. [Recognising the Structure](#5-recognising-the-structure)
8. [Finding the Exit](#6-finding-the-exit)
9. [Solving with BFS](#7-solving-with-bfs)
10. [Full Solver (PoC)](#8-full-solver-poc)
11. [Reproduction Steps](#9-reproduction-steps)
12. [Confirming on the Binary](#10-confirming-on-the-binary)
...
*Solved with `objdump`, `distorm3`, and a BFS graph traversal over the binary's address space. No decompiler needed — the node structure reads cleanly from raw disassembly, and the delta math falls out immediately from two `IMUL` instructions.*
