# FLRSCRNSVR — Reverse Engineering Writeup

> **Challenge — "FLRSCRNSVR"**
>
> *"It's just a screensaver. What could it possibly be hiding?"*
>
> **File:** `FLRSCRNSVR.SCR` (PE32+ executable (GUI) x86-64, Windows screensaver)
> **Platform:** Windows x64
> **Source:** crackme-ctf-2026 / crackmes.one
...
4. [Command-line Parsing](#2-command-line-parsing)
5. [Registry Architecture](#3-registry-architecture)
6. [Win Condition](#4-win-condition)
7. [`handling_input` Deep Dive](#5-handling_input-deep-dive)
8. [The Quak Value](#6-the-quak-value)
9. [Inverting the Transform](#7-inverting-the-transform)
10. [Full Solver (PoC)](#8-full-solver-poc)
11. [Reproduction Steps](#9-reproduction-steps)
12. [Confirming the Flag](#10-confirming-the-flag)
13. [Appendix: Key Addresses](#appendix-key-addresses)
...
**Undo step 3 — reverse again:**
Reversal is self-inverse; reversing twice returns the original order.

```python
quak.reverse()
```

**Undo step 2 — XOR again:**
XOR is self-inverse with the same key: `x ^ k ^ k == x`.
...
**Undo step 1 — inverse substitution:**
Build the reverse map `str2 → str1` and look up each character:
...
See [`solve.py`](solve.py) in this folder.
...
After XOR with index + FLARERALF and then reversal, those four values end up as the *last four* entries in Quak before reversal — i.e., positions 21–24 of the pre-reversed array — matching `[2, 14, 11, ...]` from the Quak tail. The full 25-element round-trip confirms the flag.
...
| `DAT_140008980` | screensaver_mode | Set to `1` during `/s` screensaver mode |
